...
func TestHandleCreateServerRoute(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			cognitoAttributes: []types.AttributeType{
				{
					Name:  src.stringPtr("custom:server_details"),
					Value: src.stringPtr("nil"),
				},
			},
			cognitoAttributesErr: errors.New("user unauthorized"),
		},
		{
			name:           "User already has Valheim src running",
			expectedStatus: http.StatusBadRequest,
			requiresUser:   true,
			cognitoUser: &service.CognitoUser{
				CognitoID:       "abc",
			...
			requiresCognito: true,
			requestBody:     bytes.NewBuffer([]byte(`{"world": "foo", "name": "bar", "password": "hereisapassword"}`)),
			expectedBody:    `{"error":"src:  already exists for user: foobar"}`,
			cognitoAttributes: []types.AttributeType{
				{
					Name:  src.stringPtr("custom:server_details"),
					Value: src.stringPtr("{\"world\": \"running\"}"),
				},
			},
			cognitoAttributesErr: nil,
		},
		{
			name:            "Fails to create dedicated src deployment",
			expectedStatus:  http.StatusInternalServerError,
			requiresUser:    true,
			requiresCognito: true,
			requiresKube:    true,
			...
			requestBody:  bytes.NewBuffer([]byte(`{"world": "foo", "name": "bar", "password": "hereisapassword"}`)),
			expectedBody: `{"error":"could not create dedicated src deployment: cannot validate manifest"}`,
			cognitoAttributes: []types.AttributeType{
				{
					Name:  src.stringPtr("custom:server_details"),
					Value: src.stringPtr("nil"),
		...
		},
		{
			...
			requestBody:  bytes.NewBuffer([]byte(`{"world": "foo", "name": "bar", "password": "hereisapassword"}`)),
			expectedBody: `{"error":"failed to update src details in cognito user attribute: cannot update manifest"}`,
			cognitoAttributes: []types.AttributeType{
				{
					Name:  src.stringPtr("custom:server_details"),
					Value: src.stringPtr("nil"),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockCognitoService := new(src.MockCognitoService)
