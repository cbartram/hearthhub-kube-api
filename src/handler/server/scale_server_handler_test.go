...
func TestHandleScaleServerHandlerRoute(t *testing.T) {
	tests := []struct {
		...
		{
			name:                   "No src to scale",
			expectedStatus:         http.StatusNotFound,
			requestBody:            bytes.NewBuffer([]byte(`{"replicas": 1}`)),
			expectedBody:           `{"error":"valheim src does not exist. nothing to scale."}`,
			requiresUser:           true,
			user:                   &service.CognitoUser{DiscordID: "foo", Credentials: service.CognitoCredentials{RefreshToken: "bar", AccessToken: "bar"}},
			requiresCognitoMock:    true,
		...
		},
		{
			...
			expectedStatus:         http.StatusBadRequest,
			requestBody:            bytes.NewBuffer([]byte(`{"replicas": 1}`)),
			expectedBody:           `{"error":"src already running. replicas must be 0 when src state is: RUNNING"}`,
			requiresUser:           true,
			user:                   &service.CognitoUser{DiscordID: "foo", Credentials: service.CognitoCredentials{RefreshToken: "bar", AccessToken: "bar"}},
			requiresCognitoMock:    true,
			cognitoAttributesError: nil,
		...
		},
		{
			...
			expectedStatus:         http.StatusBadRequest,
			requestBody:            bytes.NewBuffer([]byte(`{"replicas": 0}`)),
			expectedBody:           `{"error":"no src to terminate. replicas must be 1 when src state is: TERMINATED"}`,
			requiresUser:           true,
			user:                   &service.CognitoUser{DiscordID: "foo", Credentials: service.CognitoCredentials{RefreshToken: "bar", AccessToken: "bar"}},
			requiresCognitoMock:    true,
			cognitoAttributesError: nil,
